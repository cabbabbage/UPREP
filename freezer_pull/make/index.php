...
function sortCsvBySecondColumn($filePath) {
    ...
    // Sort data by the second column (index 1)
    ...
    <div class="modal-dialog modal-lg" role="document" style="max-width: 90%; margin: 0 auto; height: 80vh;"> <!-- Increased modal size -->
        <div class="modal-content custom-modal" style="color: white; background-color: black;"> <!-- Text color and background color both set to black -->
            ...
            <div class="modal-body">
                <p class="text-light" style="font-size: 18px;">Required: <?= htmlspecialchars($row[1]) ?> of <?= htmlspecialchars($row[0]) ?>. Quantity Pulled: <input inputmode="numeric" type="number" id="input<?= $index ?>" class="form-control input-remaining" data-index="<?= $index ?>" data-target="<?= htmlspecialchars($row[1]) ?>" style="height: 40px;"></p>
            </div>
